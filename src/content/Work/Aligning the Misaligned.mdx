...
As a collective dream, the OG internet is the last refuge of modernism. Caught in an architectural metaphor, it should be imagined as a huge cathedral that the entire world is building together. Everyone is welcome to join and contribute to this virtual Sacrada Familia. There is on major catch though. You have to convert to the shared belief system upon entrance. What those on the inside do not realize, however, is that their values are not universal or even universally appealing. Their dream only attracts those that are already converted.

As argued earlier, letting go of that pressure of universal values should not lead to mourning, but welcomed with a deep sigh of relief. Once we affirm and celebrate value pluralism, the internet will open up realms that go beyond modernisms wildest dreams. To unite the whole world under one roof was never its true promise. It blew up that roof altogether.  Understood as foam, the decentralized internet truly enables the creation of spaces that completely defy spatial and temporal boundaries altogether.

Balaji Srinivan's *The Network State*, may be this new era paradigm first philosophical masterpiece. Its claim is deceivingly simple: the era of the nation state is about to be replaced by the network state. In Srinivan's view, states no longer need to be defined by their claim on contiguous physical space, since the internet has removed the necessity for that requirement:
...
As it is, digital communities can already provide a lot of the functions that nation states deliver: a shared identity, a shared history, a shared currency, and they can even acquire physical territory if they want. What actually keeps a digital community from being a full-blown state, in Srinivan's view, is not territory but diplomatic recognition from other states. This, however, is not an impossibility, but simply something that has not happened yet.

The most exciting difference between nation and network states, is that the latter start from shared values rather than physical proximity.
...
Freed from the scarcity of physical space, it becomes much easier from communities of values to coexist. In fact, I would argue that they only need to respect one global principle in order to do so: the right of their citizens to vote with their feet.

## Ecosystem Architecture

More than ever before, we have the possibility to create intentional ecosystems driven by values instead of random communities created by temporal-spatio contingency.
